...
✔ O pipeline CT Scan + ViT apresentou melhor desempenho geral (~65% de acurácia)
✔ O pipeline MRI + EfficientNet-B7 mostrou alta sensibilidade para fakes, mas dificuldade com classes reais
✔ Ambas as arquiteturas conseguiram detectar com facilidade deepfakes mais artificiais (FM, FB), indicando que:

➡ Modelos generativos deixam padrões reconhecíveis, mesmo em dados médicos.
➡ Mas sinais sutis de deepfake real continuam difíceis de discriminar.
...
Os resultados mostram que a detecção é viável, mas ainda há espaço para melhorias, especialmente na diferenciação de deepfakes reais.
